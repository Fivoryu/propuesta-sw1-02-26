			...
			<div className="flex h-full flex-col rounded-xl bg-surface p-1 ring-1 ring-inset ring-line/50">
				<CardHeader className="flex-1 pb-4">
					<div className="flex items-start justify-between gap-4">
						<Badge className={accentClasses[proposal.accent]}>
							{proposal.category}
						</Badge>
					...
					<p className="pt-5 text-xs font-semibold uppercase tracking-[0.16em] text-accent">
						{proposal.eyebrow}
					</p>
					<CardTitle className="pt-1 text-2xl">{proposal.name}</CardTitle>
					<p className="max-w-prose pt-2 text-sm leading-6 text-muted">
						{proposal.summary}
					</p>
				</CardHeader>
				<CardBody className="flex flex-col gap-5">
					<div className="grid grid-cols-3 gap-2 border-y border-line/60 py-4 text-center">
						...
						<Score
							label="Innovación"
							score={proposal.criteria.innovacion.score}
						/>
