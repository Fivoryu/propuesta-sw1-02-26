...
Portal de Propuestas is the academic presentation app for the seven product prototypes in this workspace. It gives reviewers one local route for comparing the proposals, reading their boundaries, and opening each independent product entry point.

## Primary Flow

1. Open `/` to see the seven proposal cards and the simulated-prototype boundary.
...
The catalog comes from `@propuestas/shared` and `packages/shared/src/fixtures/proposal-catalog.fixture.ts`. It contains fictional proposal content only. Comparison state is held in memory and is not saved or sent anywhere. Product links read each proposal's configured `VITE_*_APP_URL` variable; an unset value falls back to `/`.
